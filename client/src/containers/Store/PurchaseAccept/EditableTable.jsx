...
class EditableTable extends React.PureComponent {
  constructor(props) {
    ...
    this.columns = [
      ...
      {
        title: '总部配送差异数量',
        dataIndex: 'dispatchDiffCount',
      },
      {
        title: '总部配送差异金额',
        dataIndex: 'dispatchDiffAmount',
      },
      {
        title: '运输差异数量',
  ...
  handleSave = (row) => {
    ...
    const calcNewData = newData.map(it => ({
      ...
      sendDiffCount: it.purchaseCount - it.dispatchDiffCount - it.acceptCount,
      sendDiffAmount: (it.purchaseCount - it.dispatchDiffCount - it.acceptCount) * it.unitPrice,
    }));
